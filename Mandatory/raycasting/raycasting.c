...
/*   Updated: 2025/02/23 02:31:25 by asalmi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	horizontal_intersection(t_game *game, double angle)
{
	...
	game->horizontal.nextHorzStepX = game->horizontal.x_intercept;
	game->horizontal.nextHorzStepY = game->horizontal.y_intercept;
	checkStepX = 0;
	checkStepY = 0;
	closer_door = false;
	while (game->horizontal.nextHorzStepX >= 0 && game->horizontal.nextHorzStepX <= game->width * UNIT_SIZE
		&& game->horizontal.nextHorzStepY >= 0 && game->horizontal.nextHorzStepY <= game->height * UNIT_SIZE)
	{
		checkStepX = game->horizontal.nextHorzStepX;
		checkStepY = game->horizontal.nextHorzStepY;
		if (is_facing_up(angle))
			checkStepY -= 1;
		...
		{
			game->horizontal.x_open = (int)(game->horizontal.nextHorzStepX / UNIT_SIZE);
			game->horizontal.y_open = (int)(game->horizontal.nextHorzStepY / UNIT_SIZE);
		...
		if (is_wall(game, checkStepX, checkStepY) || is_doors(game, checkStepX,
				checkStepY))
		{
			game->horizontal.horzWallHitX = game->horizontal.nextHorzStepX;
			game->horizontal.horzWallHitY = game->horizontal.nextHorzStepY;
			game->horizontal.foundHorzWall = true;
			break ;
		}
		game->horizontal.nextHorzStepX += game->horizontal.x_step;
		game->horizontal.nextHorzStepY += game->horizontal.y_step;
	}
}
...
{
	double	nextVertStepX;
	double	nextVertStepY;
	double	checkStepX;
	double	checkStepY;
	bool	closer_door;

	init_vertical_intersection(game, angle);
	nextVertStepX = game->vertical.x_intercept;
	nextVertStepY = game->vertical.y_intercept;
	checkStepX = 0;
	checkStepY = 0;
	closer_door = false;
...
void	find_distance(t_game *game, t_ray *ray, double angle)
{
	...
	if (game->horizontal.foundHorzWall)
		horizontal_distance = calculate_distance(game->player.position_x,
				game->player.position_y, game->horizontal.horzWallHitX,
				game->horizontal.horzWallHitY);
	if (game->vertical.foundVertWall)
		vertical_distance = calculate_distance(game->player.position_x,
				game->player.position_y, game->vertical.vertWallHitX,
				game->vertical.vertWallHitY);
